...
# Loading some of the key datasets for our analysis

`2018_def_vs_qbs` <- read.csv("2018_def_vs_qbs.txt")

`2018_passing_defense` <- read.csv("2018_passing_defense.txt")

`2018_scoring_defense` <- read.csv("2018_scoring_defense.txt")

`2018_team_defense` <- read.csv("2018_team_defense.txt")

`2019_def_vs_qbs` <- read.csv("2019_def_vs_qbs.txt")

`2019_passing_defense` <- read.csv("2019_passing_defense.txt")

`2019_scoring_defense` <- read.csv("2019_scoring_defense.txt")

`2019_team_defense` <- read.csv("2019_team_defense.txt")

`2020_team_defense` <- read.csv("2020_team_defense.txt")

`2020_passing_defense` <- read.csv("2020_passing_defense.txt")

`2020_scoring_defense` <- read.csv("2020_scoring_defense.txt")

`2020_def_vs_QBs` <- read.csv("2020_def_vs_QBs.txt")

`2021_def_vs_qbs` <- read.csv("2021_def_vs_qbs.txt")

`2021_passing_defense` <- read.csv("2021_passing_defense.txt")

`2021_scoring_defense` <- read.csv("2021_scoring_defense.txt")

`2021_team_defense` <- read.csv("2021_team_defense.txt")

# Creating lists of team abbreviations in different seasons
...
# Loading some of the key functions
...
# Using NFLfastR library to implement logos for our extra point analysis
...
# Loading the Kicking datasets from PFR

`2015.kicking.stats` <- read.csv("2015.kicking.stats.txt")
`2016.kicking.stats` <- read.csv("2016.kicking.stats.txt")
`2017.kicking.stats` <- read.csv("2017.kicking.stats.txt")
`2018.kicking.stats` <- read.csv("2018.kicking.stats.txt")
`2019.kicking.stats` <- read.csv("2019.kicking.stats.txt")
`2020.kicking.stats` <- read.csv("2020.kicking.stats.txt")
`2021.kicking.stats` <- read.csv("2021.kicking.stats.txt")

# Created a function to clean the datasets before giving appropriate abbreviations and collecting into one
...
# Now to combine the kicking datasets
...
# Let's make our graph of extra points 
...
# Now, moving onto defensive ranking comparisons between PFR's original system and my True Points
# First, we need to use our clean defense function
...
# Let's create the rank comparisons inside the dataset
...
# Just focus on the rank coordinates
...
# we can use the rank_compare dataset to make the defensive ranking changes graph to visualize the differences
...
# let's take a look at some of the defensive tiers
...
# now, we can just use the show.qs.tiers.function by selecting individual elements to display as charts
# here are Tiers 1-6
...
# QB Rushing Yards had no correlation with true points allowed in 2021-2022; but, I don't want to discount the efforts of scrambling QBs
...
`2021_josh_allen_gamelogs` <- read.csv("2021_josh_allen_gamelogs.txt")

# There's a mistake with the input, so we have to use the game log cleaner filter before 2021
...
# Going to use a function since we have to do these actions repeatedly
...
# Making another function collect the QS opportunities, too
...
# But some players don't face every tier in a season, so let's make a function to account for this situation while still representing it
...
`2021_aaron_rodgers_gamelogs` <- read.csv("2021_aaron_rodgers_gamelogs.txt")

# The mistake with gamelog_cleaner_2021 was just with Baker Mayfield's game logs, so let's keep using the earlier function
# I'll keep the updated one in case there are others with the same input error as Baker Mayfield's 2021 Season
...
`2021_trevor_lawrence_gamelogs` <- read.csv("2021_trevor_lawrence_gamelogs.txt")
...
# Let's display the game logs alongside the QS grades
# There was a possible change/mistake in some game logs for players in 2021, so I made this function to work with them
...
`2018_baker_mayfield_gamelogs` <- read.csv("2018_baker_mayfield_gamelogs.txt")

`2019_baker_mayfield_gamelogs` <- read.csv("2019_baker_mayfield_gamelogs.txt")

`2020_baker_mayfield_gamelogs` <- read.csv("2020_baker_mayfield_gamelogs.txt")

`2021_baker_mayfield_gamelogs` <- read.csv("2021_baker_mayfield_gamelogs.txt")
...
# Career Opportunities Functions
...
baker_mayfield_passing_career <- read.csv("baker_mayfield_passing_career.txt")

baker_mayfield_rushing_career <- read.csv("baker_mayfield_rushing_career.txt")
